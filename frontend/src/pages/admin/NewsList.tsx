...
type Filters = { status: string[]; category: string[]; dateFrom: string; dateTo: string };
...
export default function NewsList() {
  ...
  const [filters, setFilters] = useState<Filters>({ status: [], category: [], dateFrom: "", dateTo: "" });

  function load() {
    setLoading(true);
    api.get("/news", { params: { page: 1, limit: 100 } })
  ...
  const filtered = useMemo(() => items.filter((item) => {
    const matchSearch = !search || item.title.includes(search) || item.body?.includes(search);
    const matchStatus = filters.status.length === 0 || filters.status.includes(item.isPublished ? "published" : "draft");
  ...
  const activeFilters = filters.status.length + filters.category.length + (filters.dateFrom ? 1 : 0) + (filters.dateTo ? 1 : 0);

  function toggleFilter(key: "status" | "category", val: string) {
  ...
  async function handleDelete() {
    if (!toDelete) return;
    setDeleting(true);
    try { await api.delete(`/news/${toDelete.id}`); setToDelete(null); load(); }
    catch { setError("تعذر حذف الخبر"); }
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between pb-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">إدارة الأخبار</h1>
          <p className="mt-0.5 text-sm text-slate-500">
            {filtered.length !== items.length ? `عرض ${filtered.length} من ${items.length}` : `${items.length} خبر`}
          </p>
        </div>
        <Link to="/admin/news/new" className="rounded-lg bg-brand-600 px-4 py-2 text-sm font-semibold text-white hover:bg-brand-700">+ خبر جديد</Link>
      </div>

      <div className="mb-4 flex gap-2">
        <div className="relative flex-1">
          <Search className="pointer-events-none absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
          <input placeholder="بحث بالعنوان أو المحتوى..." value={search} onChange={(e) => setSearch(e.target.value)}
      ...
      {loading ? <Spinner /> : error ? <ErrorBox message={error} /> : items.length === 0 ? <EmptyState message="لا توجد أخبار بعد" /> : (
                    ...
                    <motion.div key={item.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18, delay: index * 0.02 }}>
                      <button type="button" onClick={() => setExpandedId(expandedId === item.id ? null : item.id)}
                        ...
                        <div className="w-20 shrink-0">
                          <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${item.isPublished ? "bg-brand-100 text-brand-700" : "bg-slate-100 text-slate-600"}`}>
                        ...
                        {expandedId === item.id && (
                          <motion.div key="exp" initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden border-t border-slate-100 bg-slate-50/70">
                            <div className="space-y-3 px-5 py-4">
                              <p className="rounded-lg bg-white p-3 text-sm leading-relaxed text-slate-600 shadow-sm line-clamp-4">{item.body}</p>
                              <div className="flex items-center justify-end gap-3 text-sm">
                                <Link to={`/admin/news/${item.id}`} className="font-medium text-brand-700 hover:underline">تعديل</Link>
              ...
              <motion.div key="fp" initial={{ width: 0, opacity: 0 }} animate={{ width: 240, opacity: 1 }} exit={{ width: 0, opacity: 0 }} transition={{ duration: 0.2 }}
                ...
                <div className="flex h-full flex-col gap-5 overflow-y-auto p-4">
                  <div className="flex items-center justify-between">
                    ...
                    {activeFilters > 0 && (
                      <button type="button" onClick={() => setFilters({ status: [], category: [], dateFrom: "", dateTo: "" })} className="flex items-center gap-1 text-xs text-slate-400 hover:text-slate-600">
                        <X className="h-3 w-3" /> مسح الكل
                      </button>
                    )}
                  </div>
                  <DateRangeFilter dateFrom={filters.dateFrom} dateTo={filters.dateTo}
                    onChange={(f, t) => setFilters((prev) => ({ ...prev, dateFrom: f, dateTo: t }))} />
                  <FilterGroup label="الحالة" options={[{ value: "published", label: "منشور" }, { value: "draft", label: "مسودة" }]}
                    selected={filters.status} onToggle={(v) => toggleFilter("status", v)} />
                  {categories.length > 0 && (
                    <FilterGroup label="التصنيف" options={categories.map((c) => ({ value: c, label: c }))}
                      selected={filters.category} onToggle={(v) => toggleFilter("category", v)} />
                  )}
                </div>
              </motion.div>
            )}
      ...
      <ConfirmDialog open={!!toDelete} title="حذف الخبر" message={`هل أنت متأكد من حذف "${toDelete?.title}"؟`}
        onConfirm={handleDelete} onCancel={() => setToDelete(null)} busy={deleting} />
    </div>
  );
}
